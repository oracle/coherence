...
public abstract class BaseTopicsTests
    ...
    static void verifyConfiguration()
        ...
        try (JavaApplication application = LocalPlatform.get().launch(JavaApplication.class,
                ClassName.of(VersionAssertion.class),
                Argument.of(CacheFactory.VERSION_ENCODED),
                Version.Previous.getClassPath()))
            {
            int exitCode = application.waitFor();
            if (exitCode != 0)
                {
                fail("The compatibility version of Coherence (" + CacheFactory.VERSION
                        + ") is not patch compatible with the Coherence version being tested.\n" +
                        "Ensure that the Coherence version configured in the coherence.compatibility.version " +
                        "property in the topics-compatibility pom.xml file is the same major version as " +
                        "the current project version.\nE.G. if the current project is 14.1.2-0-2-SNAPSHOT " +
                        "then the coherence.compatibility.version should be 14.1.2-0-x (where x is the earlier patch, " +
                        "typically patch zero).");
                }
            }
        }
