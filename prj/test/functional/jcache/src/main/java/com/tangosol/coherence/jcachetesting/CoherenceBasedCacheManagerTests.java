...
public class CoherenceBasedCacheManagerTests
    ...
    public void testJCacheNotActivatedByCacheServerActivated()
        ...
        try
            ...
            assertNull(eccf.getApplicationContext());

            // simulate DefaultCacheServer calling eccf.activate() but there is no com.tangosol.application.Context so
            // no JCache Container activation.
            eccf.activate();

            CoherenceBasedCachingProvider provider = (CoherenceBasedCachingProvider) Caching.getCachingProvider();

            assertFalse(provider.isContainerContext());
            assertEquals("validate that LifeCycleEvent.ACTIVATED occurred due to call to ECCF.activate()", 1,
                         interceptor.getActivatedCount());

            // validate that JCache Container activation did not occur since no ApplicationContext for this scenario.
            assertFalse(interceptor.isContainerContext());
            assertNull(eccf.getApplicationContext());
            assertNull(eccf.getResourceRegistry().getResource(CacheManager.class));
            }
        finally
    ...
    /**
     * Minimal simulation of Container environment within eccf.
     *
     * Note: unfortunately this may need to track implementation changes in ContainerAdapter.activate()
     *
     * @param eccf {@link com.tangosol.net.ExtensibleConfigurableCacheFactory} from LifecycleAdapter event.
     * @param cacheConfigURIString
     */
    private static void simulateContainerAdapterActivation(final ExtensibleConfigurableCacheFactory eccf,
        final String cacheConfigURIString)
        {
        // simulate Container Adapter call to activate a ECCF by adding a mock ApplicationContext to ECCF.
        // validate that JCache Container Activation performed by jcache-lifecycle-interceptor.
        Context ctxApplication = mock(Context.class);

        when(ctxApplication.getCacheConfigURI()).thenReturn(cacheConfigURIString);
        eccf.getResourceRegistry().registerResource(Context.class, ctxApplication);
        eccf.activate();
        }

    /**
     * Undo anything done in {@link #simulateContainerAdapterActivation}
     */
    private static void cleanSimulateContainer()
        {
        Caching.getCachingProvider().close();
        System.clearProperty(Constants.DEFAULT_COHERENCE_CONFIGURATION_URI_SYSTEM_PROPERTY);
        }
