 ...
 * with empty collection opposed to no response.
 * 
 * @author cp 08/14/2020
 */
public class EmptyManagementResourcesTests
    ...
    @Test
    public void testClusterWithNoCaches()
        {
        WebTarget target  = getBaseTarget().path("caches");
        Response response = target.request().get();
        assertThat(response.getStatus(), is(Response.Status.OK.getStatusCode()));
        Map<String, Object> caches = readEntity(target, response, null);
        assertThat(caches, notNullValue());
        assertThat(caches.size(), is(0));
        }
     ...
     */
    protected static String SERVER_PREFIX = "testMgmtEmptyResourceCluster";
