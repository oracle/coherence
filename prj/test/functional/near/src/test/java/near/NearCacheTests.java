...
public class NearCacheTests
    ...
    */
    ...
    public void testTruncate()
        ...
        try
            ...
            CacheFactory.log("dumping held locks after truncate front map size=" + cache.getFrontMap().size() + " controlMap size=" + cache.getControlMap().size(), 5);
        ...
        try
            ...
            arThread[1] = new Thread(()->
                                     ...
                                     try
                                         ...
                                         CacheFactory.log("thread " + this + " get(\"foo\") returned value of " + cache.get("foo"));
                                         }
                                     catch (Throwable t)
                                         {
                                         CacheFactory.log("thread " + this + " terminated with an exception. \nStack trace:\n" + Base.printStackTrace(t));
                                         }
                                     },
                                     "BlockedGetForFoo1");
            ...
            CacheFactory.log("Dump held locks before truncate");
            ((SegmentedConcurrentMap)cache.getControlMap()).dumpHeldLocks();
            ...
            try
                ...
                assertDeferred("validate truncate post condition condition that near cache control map is zero",
                                          () -> cache.getControlMap().size(), is(0),
                                          within(30, TimeUnit.SECONDS));
                }
            catch (Throwable t)
                {
                CacheFactory.log("Dump held locks after truncate and handling assertion exception:  " + t);
                ((SegmentedConcurrentMap)cache.getControlMap()).dumpHeldLocks();
    ...
    */
    ...
    protected void assertThreadStackTrace(Thread t, Class clz, String sMethodName)
        {
        assertDeferred("validate that thread " + t + " contains call to " + clz.getName() + "." + sMethodName,
