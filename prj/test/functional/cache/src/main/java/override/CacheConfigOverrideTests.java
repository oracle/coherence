...
public class CacheConfigOverrideTests
    ...
    public void testConcurrentOverride()
        ...
        try (Coherence coherence = Coherence.clusterMember().start().join())
            ...
            NamedCache elasticCache = coherence.getSession("concurrent").getCache("PagedQueue$Concurrent");
            assertNotNull(elasticCache);
            Enumeration cacheNames = elasticCache.getCacheService().getCacheNames();
            assertTrue(cacheNames.hasMoreElements());
            assertEquals("PagedQueue$Concurrent", cacheNames.nextElement());
            assertEquals("$SYS:ElasticQueue", elasticCache.getService().getInfo().getServiceName());
