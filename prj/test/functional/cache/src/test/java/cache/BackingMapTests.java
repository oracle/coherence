...
public class BackingMapTests
    ...
    public void testBackupMapPartitioned() throws Exception
     ...
     /**
      * Coh7248  Test that off-heap backup map is not partitioned by default.
      */
     @Test
     public void testOffHeapDefault() throws Exception
         {
         NamedCache cache = getNamedCache("dist-backup-nio-memory");
         TestHelper.validateBackingMapType(cache, SerializationCache.class);
         TestHelper.validateBackupMapType(cache, BinaryMap.class);
         cache.release();
         }

    /**
     * Coh7248 Test that off-heap backup map type is based on partitioned flag setting in config.
     */
    @Test
    public void testOffHeapPartitioned() throws Exception
        {
        NamedCache cache = getNamedCache("dist-backup-nio-memory-partitioned");
        TestHelper.validateBackingMapType(cache, ObservableSplittingBackingCache.class);
        TestHelper.validateBackupMapType(cache, BinaryMap.class);
        cache.release();
        }
