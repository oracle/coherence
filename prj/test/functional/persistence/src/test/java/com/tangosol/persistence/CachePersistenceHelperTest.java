...
public class CachePersistenceHelperTest
    ...
    public void testQuorumMetadataAcceptsProductMemberShape()
        ...
        when(service.getOwnershipEnabledMembers()).thenReturn(Collections.singleton(member));
    ...
    protected Member createProductMember(int nId)
        ...
        identity.setClusterName("test-cluster");
        identity.setSiteName("test-site");
        identity.setRackName("test-rack");
        identity.setMachineName("test-machine");
        identity.setProcessName("test-process");
        identity.setMemberName("test-member-" + nId);
        identity.setRoleName("storage");
        identity.setPriority(5);
        ...
        member.configure(deps, addr, 7000 + nId, 7000 + nId, new int[] {2, 1, 3});
        member.setId(nId);
        return member;
        }
