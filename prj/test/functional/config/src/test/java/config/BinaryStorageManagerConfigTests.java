...
public class BinaryStorageManagerConfigTests
    ...
    */
    ...
    /**
     * Test the default NIO memory configuration.
     */
    @Test
    public void testNioMemoryDefault() throws Exception
        {
        NamedCache cache = getNamedCache("external-nio-memory-default");
        cache.release();
        }

    /**
     * Test that the specified NIO Memory configuration is used.  There is no way
     * to check the internal NIO settings so verify that the scheme is correct.
     */
    @Test
    public void testNioMemorySpecified() throws Exception
        {
        testNio("external-nio-memory-specified", 20 * MB, 30 * MB);
        }
