...
/**
* Test for the DecoratedBinaryDeltaCompressor class.
*
* @author cp  2009.01.26
*/
public class DecoratedBinaryDeltaCompressorTest
    ...
    public void testRandom()
        ...
        do
            ...
            if (ExternalizableHelper.isDecorated(binOld) || ExternalizableHelper.isDecorated(binNew))
                {
                continue;
                }
        ...
        out("Results of DecoratedBinaryDeltaCompressorTest.testRandom():");
        out(" iters=" + cIters
    ...
    */
    private static int testExtractInternal(Binary binOld, Binary binNew)
        ...
        if (!buffersEqual(bufDelta, bufDelta3))
            {
            //XXXRHLEE
            ReadBuffer bufTemp1 = s_compressor.extractDelta(binOld, binNew);
            ReadBuffer bufTemp2 = s_compressor.extractDelta(toNonBinary(binOld), toNonBinary(binNew));
            //XXXRHLEE
            fail("binDelta=" + binToHex(bufDelta)
        ...
        return bufDelta == null ? 0 : bufDelta.length();
        }
    ...
    /**
    * The instance to test. (Other than the fact that the one uses the other,
    * DecoratedBinaryDeltaCompressor and BinaryDeltaCompressor are both
    * stateless so only one instance is required.)
    */
    public static final DecoratedBinaryDeltaCompressor s_compressor
            = new DecoratedBinaryDeltaCompressor(new BinaryDeltaCompressor());
    }
