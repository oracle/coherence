...
abstract public class AbstractRemoteFunctionTests
    ...
    protected static String createClassPathExcludingTestClasses()
        {
        ProtectionDomain domain  = AbstractRemoteFunctionTests.class.getProtectionDomain();
        ...
        if (File.separatorChar == '\\' && sClassPath.contains("\\") && sPath.contains("/"))
            {
            // handle mismatch between java.class.path using windows separator and java URL containing Unix separator
            sPath = sPath.replace("/", "\\");

            // remove leading character from windows url
            if (sPath.charAt(0) == File.separatorChar && sPath.contains(":"))
                {
                // remove leading slash from windows url
                sPath = sPath.substring(1, sPath.length() - 1);
                }
            }

        sPath = File.pathSeparator + (sPath.charAt(sPath.length() - 1) == '/' || sPath.charAt(sPath.length() -1) == '\\'
               ? sPath.substring(0, sPath.length() - 1) : sPath);

        String sResult = sClassPath.replace(sPath, "");
        if (sResult.equals(sClassPath))
            {
            System.out.println("createClassPathExcludingTestClasses: Failed to remove test component:" + sPath + " from java.class.path: " + sClassPath);
            }
        return sResult;
        }
