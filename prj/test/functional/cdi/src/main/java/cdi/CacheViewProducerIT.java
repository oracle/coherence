...
class CacheViewProducerIT
    ...
    void shouldInjectCachesFromDifferentSessions()
        {
        DifferentSessionsBean bean = weld.select(DifferentSessionsBean.class).get();
    ...
    void shouldInjectContinuousQueryCacheWithTransformerAndFilter()
        ...
        for (Map.Entry<String, Person> entry : entries)
            {
            MatcherAssert.assertThat(filtered.get(entry.getKey()), CoreMatchers.is(entry.getValue().getPhoneNumber().getNumber()));
            }
        }
    ...
    @ApplicationScoped
    private static class DifferentSessionsBean
        {
        @Inject
        @Name("numbers")
        @View
