...
public class ExtendSimilaritySearchPofIT
    ...
    static void setup()
        ...
        m_session = CLUSTER_EXTENSION.buildSession(SessionBuilders.extendClient(
                "coherence-cache-config.xml", SystemProperty.of("coherence.client", "remote")));
    ...
    public void shouldSearchAsync()
        {
        // Async will not work over Extend
        }

    static final String CLUSTER_NAME = "ExtendSimilaritySearchPofIT";

    static final int CLUSTER_SIZE = 3;

    @RegisterExtension
    static final TestLogsExtension TEST_LOGS = new TestLogsExtension(ExtendSimilaritySearchPofIT.class);

    @RegisterExtension
    static final CoherenceClusterExtension CLUSTER_EXTENSION = new CoherenceClusterExtension()
            .with(SystemProperty.of("coherence.serializer", "pof"),
