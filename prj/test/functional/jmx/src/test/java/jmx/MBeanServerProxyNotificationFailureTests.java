...
public class MBeanServerProxyNotificationFailureTests
    ...
    public void shouldRemainLiveWhenManagementSeniorRestartsDuringPofTelemetryRegistration() throws Exception
        {
        String sClusterName = System.getProperty("coherence.cluster",
                "COH33830RollingRestart-" + UUID.randomUUID());
        int    nClusterSize = 3;

        try (CoherenceCluster cluster = startCluster(sClusterName, nClusterSize,
                SystemProperty.of("test.unicast.port", Integer.getInteger("test.unicast.port", 0)),
                SystemProperty.of("test.multicast.port", Integer.getInteger("test.multicast.port", 7778)),
