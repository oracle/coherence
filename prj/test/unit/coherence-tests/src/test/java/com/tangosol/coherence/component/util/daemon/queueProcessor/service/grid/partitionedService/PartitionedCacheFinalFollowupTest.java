...
public class PartitionedCacheFinalFollowupTest
    ...
    @Test
    public void shouldSuppressPagedTopicUnpinFailureOntoProcessChangesFailure()
        {
        RuntimeException                        eProcess = new RuntimeException("process changes failed");
        RuntimeException                        eUnpin   = new RuntimeException("partition unpin failed");
        FailingCleanupPagedTopic                service  = new FailingCleanupPagedTopic(eProcess, eUnpin);
        TestGetRemainingMessagesRequest request  = new TestGetRemainingMessagesRequest();
        PartitionSet                    partMask = new PartitionSet(PARTITION_COUNT);
        ...
        RuntimeException actual = assertThrows(RuntimeException.class,
                () -> service.onGetRemainingMessagesRequest(request));

        assertSame(eProcess, actual);
        assertEquals(1, actual.getSuppressed().length);
        assertSame(eUnpin, actual.getSuppressed()[0]);
        assertEquals(1, service.getUnpinCount());
        }
    ...
    private static class TestGetRemainingMessagesRequest
            extends PagedTopic.GetRemainingMessagesRequest
        {
        @Override
        public PartitionSet getRequestMaskSafe()
            {
    ...
    private static class InvokeFailureCache
        ...
        InvokeFailureCache(Map mapPrevious)
            ...
            m_storage    = new Storage(null, null, false);
            }
        ...
        @Override
        protected Storage validateRequestForStorage(RequestMessage msgRequest, Message msgResponse,
    ...
    private static class FailingCleanupPagedTopic
            extends PagedTopic
        {
        FailingCleanupPagedTopic(RuntimeException eProcess, RuntimeException eUnpin)
            {
            super(null, null, false);

            m_eProcess = eProcess;
            m_eUnpin   = eUnpin;
            m_storage  = new PagedTopic.TopicStorage(null, null, false);
            }

        @Override
        public Message instantiateMessage(String sName)
            {
            if ("PartialValueResponse".equals(sName))
                {
                return new PartitionedCache.PartialValueResponse(null, this, true);
                }
            throw new IllegalArgumentException(sName);
            }
        ...
        private final RuntimeException       m_eProcess;
        private final RuntimeException       m_eUnpin;
        private final PagedTopic.TopicStorage m_storage;
