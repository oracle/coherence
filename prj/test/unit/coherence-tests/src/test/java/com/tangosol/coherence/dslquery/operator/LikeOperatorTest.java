...
public class LikeOperatorTest
    ...
    public void shouldHaveCorrectSymbol()
        ...
        assertThat(op.getSymbol(), is("like"));
        }
    ...
    @Test
    public void shouldCreateFilterWithoutEscapeChar()
            throws Exception
        {
        LikeOperator op       = new LikeOperator();
        ...
        Filter         expected = new LikeFilter(left, right, (char) 0, false);

        assertThat(result, is(expected));
        }

    @Test
    public void shouldCreateFilterWithEscapeChar()
            throws Exception
        {
        LikeOperator op       = new LikeOperator();
        ValueExtractor left     = new ReflectionExtractor("getFoo");
        Object[]       right    = new Object[] {"test", "\\"};
        Filter         result   = op.makeFilter(left, right);
        Filter         expected = new LikeFilter(left, "test", '\\', false);
    ...
    public void shouldBuildFilterFromTerms()
            throws Exception
        {
        Term         term     = parse("foo like 'test'");
        LikeOperator op       = new LikeOperator();
        Filter       result   = op.makeFilter(term.termAt(2), term.termAt(3), new FilterBuilder());
        Filter       expected = new LikeFilter("getFoo", "test", (char) 0, false);
