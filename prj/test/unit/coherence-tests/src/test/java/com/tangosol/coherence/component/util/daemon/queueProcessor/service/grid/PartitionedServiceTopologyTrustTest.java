...
public class PartitionedServiceTopologyTrustTest
    ...
    @Test
    public void shouldReadPersistentStoreInfoUnderBridgeFilter()
            throws Exception
        {
        PersistentStoreInfo info = new PersistentStoreInfo("store-1", false);

        try (DefaultObjectInputFilter.Scope ignored =
                DefaultObjectInputFilter.bridge(BridgeObjectInputFilter.persistentStoreInfo()))
            {
            PersistentStoreInfo result = (PersistentStoreInfo) ExternalizableHelper.readObject(writeObject(info));

            assertEquals(info.getId(), result.getId());
            assertEquals(info.isEmpty(), result.isEmpty());
            }
        }

    @Test
    public void shouldRejectUnexpectedPersistentStoreInfoBeforeCallbacks()
            throws Exception
        {
        Exploit.reset();
        ...
        try (DefaultObjectInputFilter.Scope ignored =
                DefaultObjectInputFilter.bridge(BridgeObjectInputFilter.persistentStoreInfo()))
            {
            assertRejected(() -> ExternalizableHelper.readObject(writeObject(new Exploit())));
            }
        assertFalse(Exploit.wasRead());
        }

    @Test
    public void shouldReadTransferMetadataAsBinaryUnderBridgeFilter()
            throws Exception
        {
        Binary              binary  = new Binary(new byte[] {1, 2, 3});
    ...
    @Test
    public void shouldRejectUnexpectedTransferMetadataBeforeCallbacks()
            throws Exception
        {
        Exploit.reset();
    ...
    public void shouldMaterializeTransferAddendumsAfterTopologyValidation()
        ...
        request.materializeAddendumsForTest(service);

        List listAddendum = request.getAddendums();
        assertEquals(1, listAddendum.size());
        assertTrue(listAddendum.get(0) instanceof Storage.DeferredEvent);
        assertSame(service.getLastStorage(), ((Storage.DeferredEvent) listAddendum.get(0)).getStorage());
    ...
    public static class TestTransferRequest
        ...
        void materializeAddendumsForTest(PartitionedCache service)
            {
            materializeAddendums(service);
            }

        Binary readBinaryForTest(ReadBuffer.BufferInput input)
                throws IOException
            {
            return readBinary(input, "test", false);
            }

        ReadBuffer readReadBufferForTest(ReadBuffer.BufferInput input)
                throws IOException
            {
