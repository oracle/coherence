...
public class MemcachedHelperTest
    ...
    @Test
    public void shouldRejectDangerousOptionalInnerFormat() throws IOException
        {
    ...
    @Test
    public void shouldRejectFmtExtMultiplexingJavaDelegateBeforeOrdinaryMaterialization()
            throws IOException
        {
        s_fMaterialized = false;
        Binary                 binNested = ExternalizableHelper.toBinary(new MaterializationSentinel());
        Binary                 binStored = MemcachedHelper.decorateBinary(
                fmtExtMultiplexed(DefaultSerializer.NAME, binNested), 0, 1);
        MultiplexingSerializer serializer = new MultiplexingSerializer(
                new DefaultSerializer(), new ConfigurablePofContext());

        assertEquals(ExternalizableHelper.FMT_OBJ_SER, binNested.byteAt(0) & 0xFF);
        assertThrows(RuntimeException.class, () -> ExternalizableHelper.fromBinary(binStored, serializer));
        assertFalse(s_fMaterialized);
        }

    @Test
    public void shouldRejectFmtExtMultiplexingJavaDelegateNestedExternalizableLiteFormat()
            throws IOException
        {
        Binary                 binPayload = new Binary(new byte[] {(byte) ExternalizableHelper.FMT_OBJ_EXT});
        Binary                 binStored  = MemcachedHelper.decorateBinary(
                fmtExtMultiplexed(DefaultSerializer.NAME, binPayload), 0, 1);
        MultiplexingSerializer serializer  = new MultiplexingSerializer(
                new DefaultSerializer(), new ConfigurablePofContext());
    ...
    @Test
    public void shouldAllowFmtExtMultiplexingConfiguredDelegateMaterialization()
        {
        MultiplexingSerializer serializer = new MultiplexingSerializer(
                new ConfigurablePofContext(), new DefaultSerializer());
        Binary                 binStored  = MemcachedHelper.decorateBinary(
                ExternalizableHelper.toBinary("multiplexed-pof", serializer), 0, 1);

        assertEquals("multiplexed-pof", ExternalizableHelper.fromBinary(binStored, serializer));
        }
    ...
    private static Binary fmtExtMultiplexed(String sSerializer, Binary binPayload)
            throws IOException
        {
        BinaryWriteBuffer         buffer    = new BinaryWriteBuffer(binPayload.length() + 16 + sSerializer.length());
        WriteBuffer.BufferOutput  out       = buffer.getBufferOutput();
        byte[]                    abPayload = binPayload.toByteArray();

        out.writeByte(ExternalizableHelper.FMT_EXT);
        out.writeInt(abPayload.length);
        out.writeUTF(sSerializer);
        out.write(abPayload);
