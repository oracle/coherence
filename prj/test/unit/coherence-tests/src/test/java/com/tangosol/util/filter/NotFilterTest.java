...
public class NotFilterTest
    ...
    public NotFilterTest(boolean fOrdered, boolean fPartial)
        {
        m_fOrdered = fOrdered;
        m_fPartial = fPartial;
        }
    ...
    */
    ...
    public void testApplyIndex()
        ...
        // create the NotFilter to be tested
        IndexAwareFilter filter = new NotFilter(new EqualsFilter(IdentityExtractor.INSTANCE, 3));
        ...
        // begin test
        Filter filterNew = filter.applyIndex(mapIndexes, setKeys);

        if (m_fPartial)
            {
            assertEquals(filterNew, filter);
            }
        else
            {
