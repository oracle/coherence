...
public class CacheCompositeCascadeTest
    ...
    @Test
    public void cascadesTransactionWrappers()
        {
        assertFilterRejected(new FilterWrapper<>(new CacheNamedCacheInstallGateTest.PlainFilter(), null, "test"),
                OperationReason.EVALUATE_FILTER);
        assertExtractorRejected(new ExtractorWrapper(
                new CacheNamedCacheInstallGateTest.PlainExtractor(), null, "test"), OperationReason.EXTRACT);

        RemoteInstallGate.enforceCacheFilterInstall(new FilterWrapper<>(
                new CacheNamedCacheInstallGateTest.AnnotatedFilter(), null, "test"), SerializationRole.EXTEND_PROXY,
                null);
        assertAllowed(OperationReason.EVALUATE_FILTER, 2L);

        SerializationTelemetry.resetForTesting();
        RemoteInstallGate.enforceCacheExtractorInstall(new ExtractorWrapper(
                new CacheNamedCacheInstallGateTest.AnnotatedExtractor(), null, "test"), SerializationRole.EXTEND_PROXY,
                null);
