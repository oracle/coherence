...
/**
 * Integration tests for DeepSeekModelProvider that can run against both
 * real DeepSeek API and WireMock recorded responses.
 * <p/>
 ...
 * Test modes:
 * <ul>
 * <li>Real API: When DEEPSEEK_API_KEY does not start with "test"</li>
 * <li>WireMock: When DEEPSEEK_API_KEY starts with "test" or when using recorded responses</li>
 * </ul>
 * <p/>
 * To capture new responses:
 * <ol>
 * <li>Run the Wiremock Proxy: {@code mvn exec:java@wiremock-proxy}</li>
 * <li>Set DEEPSEEK_API_KEY to a real API key</li>
 * <li>Set DEEPSEEK_BASE_URL to {@code http://localhost:8089/v1}</li>
 ...
 * @since 25.09
 */
@DisplayName("DeepSeekModelProvider Integration Tests")
class DeepSeekModelProviderIT
    {
    // ---- test lifecycle --------------------------------------------------
    ...
    static void setupClass()
        ...
        String apiKey = getProperty("deepseek.api.key", TEST_API_KEY);
        ...
        if (useWireMock)
            ...
            {
            System.out.println("Running against real DeepSeek API");
            }
        }
    ...
    void shouldReturnNullForEmbeddingModels()
        {
        // DeepSeek doesn't support embedding models
        var embeddingModel = provider.getEmbeddingModel("any-model-name");
        assertThat(embeddingModel, is(nullValue()));
        }

    // ---- inner classes ---------------------------------------------------

    /**
     * Test configuration that provides appropriate DeepSeek settings
    ...
    private static class TestConfig implements Config
        ...
        public <T> Optional<T> getOptionalValue(String propertyName, Class<T> propertyType)
            {
            if ("deepseek.api.key".equals(propertyName))
                {
                String apiKey = getProperty("deepseek.api.key", TEST_API_KEY);
                return Optional.of((T) apiKey);
                }
            else if ("deepseek.base.url".equals(propertyName))
                {
                if (useWireMock)
                    {
                    ...
                    {
                    // Use real DeepSeek API (default)
                    return Optional.ofNullable((T) getProperty("deepseek.base.url"));
                    }
                }
        ...
        public Iterable<String> getPropertyNames()
            {
            return java.util.Arrays.asList("deepseek.api.key", "deepseek.base.url");
            }
     ...
     * deepseek-chat is the standard model for most use cases.
     */
    private static final String CHAT_MODEL_NAME = "deepseek-chat";
    } 
