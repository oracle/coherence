...
public class LocalOnnxEmbeddingModel
    ...
    static PoolingConfig getPoolingConfig(ModelName name)
            throws IOException
        {
        return getPoolingConfig(name, CLIENT);
        }

    /**
     * Gets the pooling configuration using the specified web client.
     *
     * @param name    the model name
     * @param client  the web client to use for downloading
     *
     ...
     * @throws IOException  if there's an error accessing the configuration
     */
    static PoolingConfig getPoolingConfig(ModelName name, WebClient client)
            throws IOException
        {
        Path path = pathTo(name, "config.json");
        if (!Files.exists(path))
            {
            fetch(client, name, path, "1_Pooling/config.json", new MemorySize(1024));
            }
