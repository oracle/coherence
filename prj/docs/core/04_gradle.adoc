...
:coherence-gradle-plugin-version: 23.03.1
...
[IMPORTANT]
====
Due to the incomplete `Java 21` support in `Gradle 8.3` as of the release of Coherence `{coherence-version}`, we do not publish
a `23.09` version of the plugin, yet. Instead, please use the `23.03.1` version of the plugin. A new plugin version with full `Java 21`
support should be available with a future patch of Coherence `23.09` or later. However, the plugin can be used with `Java 21` projects.
====
...
As of Coherence `{coherence-version}`, `Gradle 8.3` does not have full `Java 21` support, yet.
That support is coming with `Gradle 8.4` which is currently available as a release candidate `8.4-rc-1`.

Nonetheless, `Java 21` can be used with Gradle and the Coherence Gradle plugin. In case you encounter build problems, you
have the following 2 options. Make sure that your Gradle Wrapper's `gradle-wrapper.properties` uses version `8.4-rc-1` of
Gradle:
...
You can also set up a Gradle toolchain for `Java 21`:
