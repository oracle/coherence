...
public class SerializationAllowlistTest
    ...
    public void testDevModeRejectsNonAllowlistedClass()
        {
        withProperties("dev", null, () -> assertEquals("REJECTED",
    ...
    public void testDenylistWins()
        {
        withProperties("prod", "javax.management.*", () -> assertEquals("REJECTED",
    ...
    public void testProdModeRejectsNonAllowlistedClass()
        {
        withProperties("prod", null, () -> assertEquals("REJECTED",
                check(java.io.File.class)));
        }
    ...
    public void testProdModeAcceptsExactConfiguredClass()
        {
        withProperties("prod", java.io.File.class.getName(), () -> assertEquals("ALLOWED",
                check(java.io.File.class)));
        }
    ...
    public void testProdModeAcceptsConfiguredPackageWildcard()
        {
        withProperties("prod", "java.io.*", () -> assertEquals("ALLOWED",
                check(java.io.File.class)));
        }
    ...
    public void testInvalidConfiguredEntryIsDropped()
        {
        withProperties("prod", "not a class name", () -> assertEquals("REJECTED",
                check(java.io.File.class)));
        }

    private static String check(Class<?> clz)
        {
        try
            {
            Object filter = DefaultObjectInputFilter.create();
            Object info   = newFilterInfo(clz);
            Object status = filter.getClass().getMethod("checkInput", FILTER_INFO_CLASS).invoke(filter, info);
    ...
    private static Object newFilterInfo(Class<?> clz)
        {
        return Proxy.newProxyInstance(FILTER_INFO_CLASS.getClassLoader(), new Class<?>[] {FILTER_INFO_CLASS},
                (proxy, method, args) ->
                    {
                    switch (method.getName())
                        ...
                        case "streamBytes":
                            return 0L;
                        case "toString":
                            return "TestFilterInfo(" + clz.getName() + ")";
                        case "hashCode":
    ...
    private static void assertAllowedInDevAndProd(Class<?> clz)
        ...
        for (String sMode : new String[] {"dev", "prod"})
            ...
            withProperties(sMode, null, () ->
                ...
                assertEquals(clz.getName(), "ALLOWED", check(clz));
                });
            }
        }
    ...
    private static void assertDeniedInDevAndProd(Class<?> clz)
        ...
        for (String sMode : new String[] {"dev", "prod"})
            ...
            withProperties(sMode, clz.getName(), () ->
                ...
                assertEquals(clz.getName(), "REJECTED", check(clz));
                });
            }
        }
    ...
    private static final Class<?> FILTER_INFO_CLASS = filterInfoClass();
    }
