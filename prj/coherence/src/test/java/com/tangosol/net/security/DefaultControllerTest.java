...
public class DefaultControllerTest
    ...
    private void assertProbe(int nExpected, String sMode, File fileConfig, String sExtraProperty,
        ...
        try (InputStream in = process.getInputStream())
            ...
            int    cb;
            while ((cb = in.read(abBuffer)) >= 0)
                {
                out.write(abBuffer, 0, cb);
                }
            }

        int    nExit   = process.waitFor();
        String sOutput = out.toString(StandardCharsets.UTF_8.name());
    ...
    private File rsaConfig()
        {
        return repoFile("test/functional/security/src/test/resources/DefaultControllerRsa.xml");
        }
