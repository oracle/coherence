...
import com.tangosol.internal.lucene.vectorization.VectorUtilSupport;
import com.tangosol.internal.lucene.vectorization.VectorizationProvider;

/**
 * Utilities for computations with numeric arrays, especially algebraic
 * operations like vector dot products. This class uses SIMD vectorization if
 * the corresponding Java module is available and enabled. To enable vectorized
 * code, pass {@code --add-modules jdk.incubator.vector} to Java's command
 * line.
 *
 * <p>It will use CPU's <a
 * href="https://en.wikipedia.org/wiki/Fused_multiply%E2%80%93add">FMA
 * instructions</a> if it is known to perform faster than separate multiply+add.
 * This requires at least Hotspot C2 enabled, which is the default for OpenJDK
 * based JVMs.
 *
 * <p>To explicitly disable or enable FMA usage, pass the following system
 * properties:
 *
 * <ul>
...
public final class VectorUtil
     ...
     * vector is considered "packed" (i.e. every byte representing two values).
     ...
     * @param packed   the packed vector, of length
     *                 {@code (unpacked.length + 1) / 2}
     *
     * @return the value of the dot product of the two vectors
     */
    ...
    /**
     * Dot product computed over int4 (values between [0,15]) bytes and a binary
     * vector.
     *
     ...
     * 32-bits (int) at a time. On ARM Long::bitCount is not vectorized, and
     * therefore produces less than optimal code, when compared to
     * Integer::bitCount. While Long::bitCount is optimal on x64. See
     ...
     *
     * @return A scaled score preventing negative scores for
     ...
     *
     * @throws IllegalArgumentException if any component of vector is not
     *                                  finite
     */
