...
Any example class that can cross a Coherence wire boundary must opt in to generated serialization security metadata.
Use `@Remote.Allowed` for data classes that are serialized as values, keys or messages.
Use the `Remote.Executable` annotation for entry processors, aggregators, extractors, interceptors and other classes
that Coherence executes on behalf of a client.
If a data class is already POF-managed with `@PortableType`, keep that annotation; the security-config scanner treats
it as the allow-list signal for that class.

Each example Maven build that contains wire-crossing classes should run the security config plugin:
...
The matching Gradle build applies the Coherence plugin:
...
The scanner writes `META-INF/coherence/security-config.xml` under the compiled output directory, for example
`target/classes/META-INF/coherence/security-config.xml` for Maven main classes.
If an example's security-relevant classes are test fixtures, configure the Maven plugin or Gradle `securityConfig`
extension to scan test classes as well.
For the full runtime behavior and migration details, see `docs/core/04_serialization_hardening.adoc`.
