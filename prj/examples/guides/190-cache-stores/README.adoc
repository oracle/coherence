...
Coherence supports Read-Through, Write-Through, Refresh-Ahead and Write-Behind caching.

See the https://docs.oracle.com/en/middleware/standalone/coherence/14.1.1.0/develop-applications/caching-data-sources.html#GUID-9FAD1BFB-5063-4995-B0A7-3C6F9C64F600[Coherence Documentation]
...
* <<derby-cache-store, Derby Cache Store Example>>
* <<expiring-derby-cache-store, Refresh Ahead Expiring Derby Cache Store Example>>
* <<write-behind-derby-cache-store, Write Behind Derby Cache Store Example>>
...
[#derby-cache-store]
=== Derby Cache Store Example

In this next example, we will manually create a database backed cache store using a Derby database in embedded mode.
This will show how a cache store could interact with a back-end database.

NOTE: In this example we are using an embedded Derby database just as an example and normally the back-end database would
be on a physically separate machine and not in-memory.

In this example we are storing a simple `Customer` class in our cache and cache-store. Continue below to review the `DerbyCacheStore` class.

*Review the DerbyCacheStore*

1. Specify the class that implements the `CacheStore` interface
+
[source,java,indent=0]
----
include::src/main/java/com/oracle/coherence/guides/cachestores/DerbyCacheStore.java[tag=class]
----
...
[source,java,indent=0]
----
include::src/main/java/com/oracle/coherence/guides/cachestores/DerbyCacheStore.java[tag=constructor]
----
...
[source,java,indent=0]
----
include::src/main/java/com/oracle/coherence/guides/cachestores/DerbyCacheStore.java[tag=load]
----
...
[source,java,indent=0]
----
include::src/main/java/com/oracle/coherence/guides/cachestores/DerbyCacheStore.java[tag=store]
----
...
[source,java,indent=0]
----
include::src/main/java/com/oracle/coherence/guides/cachestores/DerbyCacheStore.java[tag=storeInternal]
----
...
[source,java,indent=0]
----
include::src/main/java/com/oracle/coherence/guides/cachestores/DerbyCacheStore.java[tag=storeAll]
----
+
...
[source,java,indent=0]
----
include::src/main/java/com/oracle/coherence/guides/cachestores/DerbyCacheStore.java[tag=erase]
----

*Review the Cache Configuration*

Review the Cache Configuration `derby-cache-store-cache-config.xml`

[source,xml,indent=0]
----
include::src/main/resources/derby-cache-store-cache-config.xml[tag=config]
----
<1> Cache mapping for Customer cache to the `derby-cache-loader` scheme
<2> Cache mapping for CustomerExpiring cache to the `derby-cache-loader` scheme (see next section)
...
Next we will run the `DerbyCacheStoreTest.java` unit test below and observe the behaviour.

1. Start and confirm NamedMap and database contents.
+
[source,java,indent=0]
----
include::src/test/java/com/oracle/coherence/guides/cachestores/DerbyCacheStoreTest.java[tag=initial]
----
...
[source,java,indent=0]
----
include::src/test/java/com/oracle/coherence/guides/cachestores/DerbyCacheStoreTest.java[tag=load1]
----
+
...
[source,java,indent=0]
----
include::src/test/java/com/oracle/coherence/guides/cachestores/DerbyCacheStoreTest.java[tag=load2]
----
+
...
[source,java,indent=0]
----
include::src/test/java/com/oracle/coherence/guides/cachestores/DerbyCacheStoreTest.java[tag=remove]
----
...
[source,java,indent=0]
----
include::src/test/java/com/oracle/coherence/guides/cachestores/DerbyCacheStoreTest.java[tag=update]
----
...
[source,java,indent=0]
----
include::src/test/java/com/oracle/coherence/guides/cachestores/DerbyCacheStoreTest.java[tag=addRemove]
----
...
[source,java,indent=0]
----
include::src/test/java/com/oracle/coherence/guides/cachestores/DerbyCacheStoreTest.java[tag=loadData]
----

[#expiring-derby-cache-store]
=== Refresh Ahead Derby Cache Store Example
...
The `derby-cache-store-cache-config.xml` below shows the `CustomerExpiring` cache passing in parameters to the `caching-scheme` to override
expiry and refresh ahead values.

[source,xml,indent=0]
----
include::src/main/resources/derby-cache-store-cache-config.xml[tag=refresh]
----
...
[source,xml,indent=0]
----
include::src/main/resources/derby-cache-store-cache-config.xml[tag=local]
----
...
[source,xml,indent=0]
----
include::src/main/resources/derby-cache-store-cache-config.xml[tag=refresh2]
----

*Run the Unit Test*

Next we will run the `DerbyCacheStoreExpiringTest.java` unit test below and observe the behaviour.

1. Start and confirm NamedMap and database contents.
+
[source,java,indent=0]
----
include::src/test/java/com/oracle/coherence/guides/cachestores/DerbyCacheStoreExpiringTest.java[tag=initial]
----
...
[source,java,indent=0]
----
include::src/test/java/com/oracle/coherence/guides/cachestores/DerbyCacheStoreExpiringTest.java[tag=readThrough1]
----
+
...
[source,java,indent=0]
----
include::src/test/java/com/oracle/coherence/guides/cachestores/DerbyCacheStoreExpiringTest.java[tag=readThrough2]
----
+
...
[source,java,indent=0]
----
include::src/test/java/com/oracle/coherence/guides/cachestores/DerbyCacheStoreExpiringTest.java[tag=readThrough3]
----
+
...
[#write-behind-derby-cache-store]
=== Write Behind Derby Cache Store Example

In this final Derby cache store example, we use the `CustomerWriteBehind` cache which has a write delay of 10 seconds.

*Review the Cache Configuration*

The `derby-cache-store-cache-config.xml` below shows the `CustomerWriteBehind` cache passing in parameters to the `caching-scheme` to override
`write-delay` value.

[source,xml,indent=0]
----
include::src/main/resources/derby-cache-store-cache-config.xml[tag=writeBehind]
----

*Run the Unit Test*

Next we will run the `DerbyCacheStoreExpiringTest` unit test below and observe the behaviour.

1. Start and confirm NamedMap and database contents. In this example we are not preloading the database.
+
[source,java,indent=0]
----
include::src/test/java/com/oracle/coherence/guides/cachestores/DerbyCacheStoreWriteBehindTest.java[tag=initial]
----
...
[source,java,indent=0]
----
include::src/test/java/com/oracle/coherence/guides/cachestores/DerbyCacheStoreWriteBehindTest.java[tag=insert]
----
...
[source,java,indent=0]
----
include::src/test/java/com/oracle/coherence/guides/cachestores/DerbyCacheStoreWriteBehindTest.java[tag=wait]
----
+
...
=== See Also
* https://docs.oracle.com/middleware/12213/coherence/develop-applications/caching-data-sources.htm#COHDG5177[Caching Data Stores]
