...
In this final HSQLDb cache store example, we use the `CustomerWriteBehind` cache which has a write delay of 10 seconds.
...
Next we will run the `HSqlDbCacheStoreExpiringTest` unit test below and observe the behaviour.
