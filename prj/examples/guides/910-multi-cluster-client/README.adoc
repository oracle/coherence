...
Creating a gRPC client session is currently slightly more complicated than an Extend session.
The Coherence gRPC API is configured in code and not quite as flexible as configuring Extend. This will be changed in future releases to make gRPC as easy to use as Extend.

The basic steps are the same as above for an Extend client, but for gRPC an `io.grpc.Channel` instance must be provided to the session configuration.

A gRPC `Channel` can be created using the `grpc-java` APIs as shown below.
This example creates a channel that uses insecure credentials, other options are available to create secure TLS channels.
...
Once a channel has been created, the gRPC session configuration can be created:
...
This can be used to create a gRPC session for a tenant, as shown below:
...
public Session getSession(String tenant)
    ...
    ManagedChannel channel = Grpc.newChannelBuilderForAddress(hostName, port, credentials) // <4>
            .build();

    coherence.addSessionIfAbsent(tenant, () ->     // <5>
            GrpcSessionConfiguration.builder(channel)
                    .named(tenant)
                    .withSerializerFormat(metaData.getSerializer())
                    .build());

    return coherence.getSession(tenant);  // <6>
    }
----
...
<2> Find the `Session` with the tenant name
<3> The `Session` has already been created, so use it
<4> Create the `Channel`, the host name and port and credentials would probably be looked up and be specific for the tenant,
in this example it is a simple in-secure connection to `127.0.0.1:1408`.
<5> Use the `Coherence.addSessionIfAbsent()` method to add a `GrpcSessionConfiguration` for the tenant.
<6> Return the gRPC `Session` for the configuration name just added
...
Creating a gRPC `Session` is slightly more complex than an Extend `Session`.
The complication comes from having to manually create a gRPC `ManagedChannel` to pass to the `GrpcSessionConfiguration`.
Eventually the dynamic configuration of gRPC clients will be simplified in Coherence, but currently this is the method that works.
...
<1> The host name of the tenant's cluster is obtained from the meta-data
<2> The port for the gRPC proxy in the tenant's cluster is obtained from the meta-data
<3> The `ChannelCredentials` are created to configure gRPC security, in this case an insecure (non-TLS) connection is used
<4> A `ManagedChannel` is created using the host name, port and credentials.
<5> A `GrpcSessionConfiguration.Builder` is created using the channel
<6> The name of the session is set to the tenant name
<7> The name of the serializer is set, in this example it will be Java serialization, but "pof" is also supported.
<8> The `GrpcSessionConfiguration` is built and returned

==== Summary

The example code could be simplified if the application only ever used Extend or only ever used gRPC.
There are also many alternative approaches to holding tenant metat data used to create the sessions.
