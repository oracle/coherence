...
public class TopicsChatApplicationTest {
    ...
    @BeforeAll
    static void boostrapCoherence() throws Exception {
    ...
    public void testChatApplication() throws IOException {
        ...
        Eventually.assertDeferred(user1::isReady, is(true));

        executor.execute(user2);
        Eventually.assertDeferred(user2::isReady, is(true));

        // send some commands to user1
        pos1.write("send hello\n".getBytes());
        Eventually.assertDeferred(user1::getMessagesSent, is(1));

        // user2 will get the join and the message from user 1
        Eventually.assertDeferred(user2::getMessagesReceived, is(2));

        // send command to user2
        pos2.write("send hello\n".getBytes());

        // user2 has now sent 1 message
        Eventually.assertDeferred(user2::getMessagesSent, is(1));

        // user1 had now received one message
        Eventually.assertDeferred(user1::getMessagesReceived, is(2));
        ...
        Eventually.assertDeferred(user3::isReady, is(true));

        // user1 and 2 will receive a join message
        Eventually.assertDeferred(user1::getMessagesReceived, is(3));
        Eventually.assertDeferred(user2::getMessagesReceived, is(3));

        // send pm from user-3 to user-1
        pos3.write("sendpm user-1 hello\n".getBytes());

        // user-1 should get message
        Eventually.assertDeferred(user1::getMessagesReceived, is(4));
