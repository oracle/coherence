...
----
<1> Get the `countries` Map
<2> We create the same `GreaterEqualsFilter` as in the previous test (Select countries with more than 60 million people, only)
...
<1> Get the `countries` Map
<2> We create the same `GreaterEqualsFilter` as in the previous test (Select countries with more than 60 million people, only)
<3> We will use a different `Aggregator`. `BigDecimalSum` will aggregate the population and return a `Bigecimal` value
<4> Apply the `Filter` and `Aggregator`
