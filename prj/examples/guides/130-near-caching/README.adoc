...
See the https://docs.oracle.com/en/middleware/standalone/coherence/14.1.1.0/develop-applications/introduction-coherence-caches.html[Coherence Documentation]
...
* Configuring expiry and eviction policies
...
as well as optionally with an expiry. The back cache is a distributed cache.
...
* Displays `Cache` MBean metrics for the front cache
...
* Displays `Cache` MBean metrics for the front cache to show cache pruning happening
* Displays `StorageManager` MBean metrics to show listener registrations
//-

There are three tests that exercise the above `SimpleNearCachingExample` class and using different caches
...
front limit to 100
<2> Define cache mapping for caches matching `expiring-cache-*` to the `near-scheme` using macros to set the front limit
to 100, and the expiry to 8 seconds
<3> Define an eviction policy to apply when `high-units` are reached
<4> Define front scheme `high-units` using the macro and defaulting to 10 if not set
<5> Define front scheme `expiry-delay` using the macro and defaulting to 0s if not set
<6> Define back scheme as standard distributed scheme
<7> System property to set the invalidation strategy for each test
...
<3> Sleep for 5 seconds to ensure JMX stats are up to date as well as expire entries second time around if the expiring cache is being used
...
* SimpleExpiringNearCachingExampleTest - uses `all` invalidation strategy and front expiry of 3 seconds

//-
There are a number of invalidation strategies, described https://docs.oracle.com/pls/topic/lookup?ctx=en/middleware/standalone/coherence/14.1.1.0/install&id=COHDG5219[here],
...
3. Review the `SimpleExpiringNearCachingExampleTest`
+
[source,java,indent=0]
----
include::src/test/java/com/oracle/coherence/guides/nearcaching/SimpleExpiringNearCachingExampleTest.java[tag=test]
----
+
NOTE: This test runs with a cache called `expiring-cache-all`, which matches the size limited and expiring near cache and
invalidation strategy of `all`.
Due to the expiry behaviour, the output will be different from the size limited cache.

[#run-example-1]
=== Run the Examples

Run the examples using one of the methods below:
...
*SimpleExpiringNearCachingExampleTest Output*

The output of this test is slightly different from the previous two as and expiring cache is used.
See below for details of the output.

[source,bash]
----
<Info> (thread=main, member=1): Running test with cache expiring-cache-all
<Info> (thread=main, member=1): Iteration #1 Total time for gets 21.364ms <1>
<Info> (thread=main, member=1): Coherence:type=Cache,service=DistributedCache,name=expiring-cache-all,nodeId=1,tier=front,loader=414493378 <2>
<Info> (thread=main, member=1): Name: TotalGets, value=100
<Info> (thread=main, member=1): Name: TotalPuts, value=100
<Info> (thread=main, member=1): Name: CacheHits, value=0
<Info> (thread=main, member=1): Name: Size, value=100
...
<Info> (thread=main, member=1): Name: CachePrunes, value=0
<Info> (thread=main, member=1): Iteration #2 Total time for gets 0.543ms <3>
...
<Info> (thread=main, member=1): Name: Size, value=10 <5>
<Info> (thread=main, member=1): Name: HitProbability, value=0.5238095238095238
<Info> (thread=main, member=1): Name: AverageMissMillis, value=0.2
<Info> (thread=main, member=1): Name: CachePrunes, value=0 <6>
<Info> (thread=main, member=1): Coherence:type=StorageManager,service=DistributedCache,cache=expiring-cache-all,nodeId=1
<Info> (thread=main, member=1): Name: ListenerRegistrations, value=1 <7>
<Info> (thread=main, member=1): Name: InsertCount, value=110
----
<1> Iteration #1 for gets takes 21.364ms which includes the time to populate the front cache
<2> The Cache MBean object name for the front cache and various metrics
<3> Iteration #2 for gets takes only 0.543ms which is considerably quicker due to the entries being in the front cache
<4> The Hit Probability is 0.5 or 50% as 100 out of 200 entries were read from the front cache
<5> After the extra puts and gets, we can see the size is 10 as the total sleep time was > 8 seconds which meant
the near cache expired 100 entries before the next 10 entries were added
<6> Number of prune operations are 0 as no size limiting pruning has been done
<7> Because we are using the `all` invalidation strategy there is only 1 listener registered for all the entries
...
* Configured expiry and eviction policies
* Explored MBeans related to near caching

[#see-also]
=== See Also

* https://docs.oracle.com/en/middleware/standalone/coherence/14.1.1.0/develop-applications/introduction-coherence-caches.html#GUID-5C066CC9-575F-4D7D-9D53-7BB674D69FD1[Understanding Near Caches]
* https://docs.oracle.com/en/middleware/standalone/coherence/14.1.1.0/develop-applications/configuring-caches.html#GUID-F91E64DD-2C46-4ED9-BD41-04D2922312F6[Defining Near Cache Schemes]
* https://docs.oracle.com/pls/topic/lookup?ctx=en/middleware/standalone/coherence/14.1.1.0/install&id=COHDG5219[Near Cache Invalidation Strategies]
* https://docs.oracle.com/en/middleware/standalone/coherence/14.1.1.0/develop-applications/introduction-coherence-caches.html#GUID-9E6ABD8C-AD27-48C7-9C57-2A90133CEB3A[Understanding Local Caches]
* https://docs.oracle.com/en/middleware/standalone/coherence/14.1.1.0/develop-applications/cache-configuration-elements.html#GUID-2DA5531C-4D2B-4582-9ED7-012120122BB9[Near Cache local-scheme Configuration]
* https://docs.oracle.com/en/middleware/standalone/coherence/14.1.1.0/administer/performance-tuning.html#GUID-20600648-0C50-4275-9AE2-782CE32CAC2D[Near Cache and Cluster-node Affinity]
* https://docs.oracle.com/en/middleware/standalone/coherence/14.1.1.0/administer/performance-tuning.html#GUID-068ABDF6-19A2-4C54-8B5B-8D1059EFFFC7[Concurrent Near Cache Misses on a Specific Hot Key]
