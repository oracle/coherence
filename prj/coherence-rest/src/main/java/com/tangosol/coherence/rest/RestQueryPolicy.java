...
public final class RestQueryPolicy
    ...
    public static Filter createNamedQueryFilter(String sExpression, Map<String, Object> mapParams)
        {
        // named-query SSE stays on generic CohQL because the expression is operator-configured, not HTTP direct-query text
        // this policy helper gives future SSE/query policy work one hook while preserving configured named-query compatibility
    ...
    private static void validateIdentifier(Term term)
        ...
        if (isBlank(sIdentifier) || sIdentifier.indexOf('.') >= 0)
            {
    ...
    private static boolean isNumericLiteral(Term term)
        {
        // unary +/- is only safe for numeric constants, and OPParser wraps those constants as literal(number)
        if (term instanceof AtomicTerm)
            {
    ...
    private static String stripAndValidateTypeHints(String sQuery)
        {
        // REST type hints stay as a single token; rejecting
        // whitespace-split hints here prevents validation from silently dropping
        // a trailing ;type fragment that execution could parse differently
     ...
     */
    private static final Set<String> ALLOWED_OPERATORS = immutableSet(
            "&&", "||",
            "==", "!=", "<", "<=", ">", ">=",
     ...
     */
    private static final Set<String> ALLOWED_ATOMS = immutableSet(
     ...
     */
    private static final Set<String> ALLOWED_TYPE_HINTS = immutableSet(
            "i", "int", "s", "short", "l", "long", "f", "float",
            "d", "double", "I", "BigInteger", "D", "BigDecimal",
            "date", "uuid", "uid");

    /**
     * Create an immutable set from the supplied values.
     *
     * @param asValue  the values
     *
     * @return an immutable set containing the values
     */
    private static Set<String> immutableSet(String... asValue)
        {
        return Collections.unmodifiableSet(new HashSet<String>(Arrays.asList(asValue)));
        }
