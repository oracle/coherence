...
public class SecurityFilter implements ContainerRequestFilter
    ...
    public void filter(ContainerRequestContext filterContext) throws IOException
        ...
        if (securityContext != null)
            ...
            {
            // fail closed only after the hosting server/container has engaged auth
    ...
    private SecurityContext authenticate(ContainerRequest request, RestSecurityPolicy.AuthenticationState authState)
        ...
        // reuse the safely evaluated state to avoid calling container accessors again
        ...
        if (fAuthEngaged && CoherenceMode.isCoherenceRestAuthEnforced())
            {
            // makes the container principal the trust signal; headers alone must not authenticate DEV/PROD REST
        ...
        if (!fAuthEngaged && CoherenceMode.isCoherenceRestAuthEnforced())
            {
            // preserves no-auth anonymous resources by ignoring spoofed auth headers
            return null;
            }
        ...
        // the HTTP server already authenticated this legacy basic user
