...
public final class JsonClassMetadataPolicy
    ...
    public static Class<?> resolveClassLiteral(String sClassName, ClassLoader loader)
        ...
        // design/features/security-bugs/plans/rest-01/prompts/05-slice-d-json-class-metadata-implementation.md requires
        // DEV/PROD to deny remote Class literals because there is no caller trust signal here
    ...
    private static Class<?> resolveConfiguredAlias(String sAlias,
        ...
        if (clz == null)
            {
            // design/features/security-bugs/plans/rest-01/prompts/05-slice-d-json-class-metadata-implementation.md keeps
            // DEV/PROD on configured aliases instead of falling through to Class.forName
        ...
        if (!SerializationGate.isValid(clz))
            {
            // rest-01 prompt 05 allows configured aliases only when the target class also passes the shared serialization gate
