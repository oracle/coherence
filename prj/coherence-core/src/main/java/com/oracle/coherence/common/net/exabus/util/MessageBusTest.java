...
public class MessageBusTest
    {
    /**
     * Return the number of blocking collector invocations completed by the functional test hook.
     *
     * @return the number of completed blocking collector invocations
     */
    public static int getBlockingCollectorInvocationCountForTesting()
        {
        return TEST_BLOCKING_COLLECTOR_INVOCATIONS.get();
        }

    /**
     * Return the number of collector reentry attempts that timed out.
     *
     * @return the number of timed-out collector reentry attempts
     */
    public static int getCollectorReentryTimeoutCountForTesting()
        {
        return TEST_COLLECTOR_REENTRY_TIMEOUTS.get();
        }
    ...
    public static class EventProcessor
        ...
        /**
         * Return the number of primary request messages sent by this processor.
         *
         * @return the number of primary request messages sent
         */
        public long getPrimaryMessagesOut()
            {
            return m_cPrimaryOut;
            }

        /**
         * Return the number of response messages generated by this processor.
         *
         * @return the number of generated response messages
         */
        public long getGeneratedResponses()
            {
            return m_cResponseOut;
            }
        ...
        /**
         * The number of primary request messages sent.
         */
        protected long m_cPrimaryOut;

        /**
         * The number of response messages generated.
         */
        protected long m_cResponseOut;
    ...
    public static class DemultiplexingCollector implements Collector<Event>
        ...
        public void add(Event event)
            ...
            if (event.getType() == Event.Type.MESSAGE &&
                TEST_COLLECTOR_BLOCK_MILLIS > 0L &&
                TEST_BLOCKING_COLLECTOR_STARTED.compareAndSet(false, true))
                {
                CountDownLatch latchReentered = new CountDownLatch(1);
                Thread thread = new Thread(() ->
                    ...
                    try
                        ...
                        {
                        latchReentered.countDown();
                        }
                    }, "MessageBusTestCollectorReentry");
                thread.setDaemon(true);
                thread.start();

                try
                    {
                    if (!latchReentered.await(5L, TimeUnit.SECONDS))
                        {
                        TEST_COLLECTOR_REENTRY_TIMEOUTS.incrementAndGet();
                        }
                    Blocking.sleep(TEST_COLLECTOR_BLOCK_MILLIS);
                    }
                    ...
                    {
                    TEST_BLOCKING_COLLECTOR_INVOCATIONS.incrementAndGet();
                    }
                }
    ...
    public static void main(String[] asArg)
        ...
        for (int iReport = 0; ; ++iReport)
            ...
            for (EventProcessor proc : aProcessor)
                ...
                stats.cPrimaryOut          += proc.getPrimaryMessagesOut();
                stats.cResponseOut         += proc.getGeneratedResponses();
            ...
            for (Transmitter trans : aTransmitter)
                ...
                stats.cPrimaryOut          += trans.getMessagesOut();
            ...
            for (Stats statsComp : new Stats[]{statsPrev, statsWarm})
                ...
                long cPrimaryOutDelta          = stats.cPrimaryOut          - statsComp.cPrimaryOut;
                long cResponseOutDelta         = stats.cResponseOut         - statsComp.cResponseOut;
                ...
                long PRIMARYsOut = Math.round(cPrimaryOutDelta  / dflSeconds);
                long RESPONSEsOut = Math.round(cResponseOutDelta / dflSeconds);
                ...
                if (fTabular)
                    ...
                    System.out.println((statsComp == statsPrev ? "now:  " : "life: ") +
                            ...
                            ", primary " + PRIMARYsOut + "msg/s, responses " + RESPONSEsOut + "msg/s), " +
                            "latency(" +
            ...
            if (s_fVerbose)
                ...
                if (AbstractSocketBus.isTransportMetricsTrackedForTesting())
                    {
                    long cHolds = AbstractSocketBus.getConnectionLockHoldCountForTesting();
                    long cNanos = AbstractSocketBus.getConnectionLockHoldNanosForTesting();
                    System.out.println("transport metrics: lock holds=" + cHolds +
                            ", avg lock hold nanos=" + (cHolds == 0L ? 0L : cNanos / cHolds) +
                            ", max lock hold nanos=" + AbstractSocketBus.getConnectionLockHoldMaxNanosForTesting() +
                            ", max selector task delay nanos=" + AbstractSocketBus.getSelectorTaskDelayMaxNanosForTesting() +
                            ", max migration completion nanos=" + AbstractSocketBus.getMigrationCompletionMaxNanosForTesting());
                    }
                }
    ...
    /**
     * The duration for which the first message collector call blocks during functional testing.
     */
    private static final long TEST_COLLECTOR_BLOCK_MILLIS = Long.getLong(
            MessageBusTest.class.getName() + ".collectorBlockMillis", 0L);

    /**
     * The number of blocking collector calls completed by the functional test hook.
     */
    private static final AtomicInteger TEST_BLOCKING_COLLECTOR_INVOCATIONS = new AtomicInteger();

    /**
     * Whether the blocking collector hook has started.
     */
    private static final AtomicBoolean TEST_BLOCKING_COLLECTOR_STARTED = new AtomicBoolean();

    /**
     * The number of collector reentry attempts that could not complete while the collector was blocked.
     */
    private static final AtomicInteger TEST_COLLECTOR_REENTRY_TIMEOUTS = new AtomicInteger();
