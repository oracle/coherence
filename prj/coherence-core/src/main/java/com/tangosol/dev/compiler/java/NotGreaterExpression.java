...
/**
* Implements the less than or equals to (<=) relational operator.
*
* @version 1.00, 10/05/98
* @author  Cameron Purdy
*/
public class NotGreaterExpression extends RelationalExpression
    {
    // ----- construction ---------------------------------------------------

    /**
    * Construct a NotGreaterExpression.
    *
    * @param left      the left expression
    * @param operator  the operator token
    * @param right     the right expression
    */
    public NotGreaterExpression(Expression left, Token operator, Expression right)
        {
    ...
    protected boolean compile(Context ctx, CodeAttribute code, boolean fReached, ErrorList errlist)
        ...
        if (!ctx.isDebug() && isConstant())
            ...
            switch (left.getType().getTypeString().charAt(0))
                {
                case 'I':
                    code.add(new If_icmple(lblTrue));
                    break;

                case 'J':
                    code.add(new Lcmp());
                    code.add(new Ifle(lblTrue));
                    break;
    ...
    * The class name.
    */
    private static final String CLASS = "NotGreaterExpression";
    }
