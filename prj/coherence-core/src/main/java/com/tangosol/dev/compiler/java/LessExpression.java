...
*/
public class LessExpression extends RelationalExpression
    {
    // ----- construction ---------------------------------------------------

    /**
    * Construct a LessExpression.
    *
    * @param left      the left expression
    * @param operator  the operator token
    * @param right     the right expression
    */
    public LessExpression(Expression left, Token operator, Expression right)
        {
    ...
    protected boolean compile(Context ctx, CodeAttribute code, boolean fReached, ErrorList errlist)
        ...
        if (!ctx.isDebug() && isConstant())
            ...
            switch (left.getType().getTypeString().charAt(0))
                {
                case 'I':
                    code.add(new If_icmplt(lblTrue));
                    break;

                case 'J':
                    code.add(new Lcmp());
                    code.add(new Iflt(lblTrue));
                    break;
    ...
    * The class name.
    */
    private static final String CLASS = "LessExpression";
    }
