    ...
    */
    ...
    public void restoreState(Object state)
        ...
        if (!(state instanceof Object[]) || ((Object[]) state).length != 3)
            {
            throw new IllegalArgumentException("invalid long maximum state snapshot");
            }
        ...
        if (cValues < 0 || cSupport < 0 || cSupport > cValues)
            {
            throw new IllegalArgumentException("invalid long maximum state values");
            }
        ensureInitialized(false);
        m_count    = cValues;
    ...
    public int compareContinuousAggregationBounds(Object left, Object right)
        {
        return Long.compare(((Number) right).longValue(), ((Number) left).longValue());
        }
