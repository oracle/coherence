    ...
    */
    ...
    public void restoreState(Object state)
        ...
        if (!(state instanceof Object[]) || ((Object[]) state).length != 3)
            {
            throw new IllegalArgumentException("invalid double minimum state snapshot");
            }
        ...
        if (cValues < 0 || cSupport < 0 || cSupport > cValues)
            {
            throw new IllegalArgumentException("invalid double minimum state values");
            }
        ensureInitialized(false);
        m_count     = cValues;
    ...
    */
    protected void process(Object o, boolean fFinal)
        ...
        if (o != null)
            ...
            double dflResult = m_count == 0 ? dflValue : Math.min(m_dflResult, dflValue);
