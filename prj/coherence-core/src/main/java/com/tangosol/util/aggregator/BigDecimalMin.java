    ...
    */
    ...
    public void restoreState(Object state)
        ...
        if (!(state instanceof Object[]) || ((Object[]) state).length != 3)
            {
            throw new IllegalArgumentException("invalid BigDecimal minimum state snapshot");
            }
        ...
        if (cValues < 0 || cSupport < 0 || cSupport > cValues
            ...
            {
            throw new IllegalArgumentException("invalid BigDecimal minimum state values");
            }
        ensureInitialized(false);
        m_count     = cValues;
