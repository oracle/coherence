...
public final class SerializationAllowlist
    ...
    private static boolean isValidEntry(String sEntry, boolean fPrefix)
        ...
        if (!fValid && !fPrefix && !PATTERN_PACKAGE_WILDCARD.matcher(sEntry).matches())
            {
            Logger.warn(String.format("Ignored serialization allowlist entry: property=%s, reason=%s, entry=%s",
                    PROP_SERIALIZATION_ALLOWED, "serialization-allowlist-entry-invalid", boundedValue(sEntry)));
            }
        return fValid;
        }
    ...
    private static Class<?> validateRestGeneratedPartialClass(MethodHandles.Lookup lookup, Class<?> clz)
        ...
        // private access is the strongest caller proof available on this Java 8 release line
    ...
    private static boolean isTrustedRestPartialObject(Class<?> clz)
        ...
        try
            {
            // REST-01 prompt 09 projection alias allowlist follow-up: unnamed
            // deployments fail closed unless core resolves the exact REST anchor
    ...
    private static class Allowlist
        {
        Allowlist(String sRaw, Set<String> setExact, Set<String> setPrefix)
            {
            m_sRaw      = sRaw;
            m_setExact  = setExact;
        ...
        boolean matches(String sName)
            {
            return m_setExact.contains(sName) || matchesPrefix(sName, m_setPrefix);
            }

        private final String      m_sRaw;
        private final Set<String> m_setExact;
        private final Set<String> m_setPrefix;
        }
     ...
     */
    private static final Set<String> BASELINE_EXACT = immutableSet(
            "java.lang.Number",
            "java.lang.Object",
            "java.lang.String",
            "java.lang.Boolean",
     ...
     */
    private static final Set<String> BASELINE_PREFIX = immutableSet(
            "java.util.",
            "java.time.",
            "java.math.",
     ...
     */
    private static final Set<String> DENY_EXACT = immutableSet(
     ...
     * Package deny-list.
     */
    private static final Set<String> DENY_PREFIX = immutableSet(
            "com.sun.jndi.",
            "java.rmi.server.",
    ...
    private static Set<String> immutableSet(String... asValue)
        ...
        Collections.addAll(set, asValue);
