...
public class ExtensibleConfigurableCacheFactory
    ...
    private <K, V> NamedCache<K, V> ensureCacheInternal(String               sCacheName,
        ...
        while (true)
            ...
            if (context != null)
                ...
                if (contextCurrent == null ||
                   !contextCurrent.getDomainPartition().equals(context.getDomainPartition()))
                    {
                    throw new IllegalStateException("ensureCache() called out of context");
                    }

                if (!context.isGlobalDomainPartition() && context.isCacheShared(sCacheName))
                    {
            ...
            try
                {
                resolver     = getParameterResolver(sCacheName, CacheMapping.class, loader, null, context);
        ...
        while (true)
            ...
            if (col != null && col.isActive())
                ...
                if (clsCollection.isAssignableFrom(col.getClass()))
                    {
                    ContainerContext ctx = ContainerHelper.getSwitchServiceContext(col.getService());
                    if (ctx == null)
                        {
                        checkPermission(col);
                        }
                        ...
                        {
                        final C colTmp = col;
                        ctx.runInDomainPartitionContext(() -> checkPermission(colTmp));
                        }
            ...
            if (context != null)
                ...
                if (contextCurrent == null ||
                   !contextCurrent.getDomainPartition().equals(context.getDomainPartition()))
                    {
                    throw new IllegalStateException("ensureCollection() called out of context");
                    }

                if (!context.isGlobalDomainPartition() && context.isCacheShared(sName))
                    {
                    context = context.getGlobalContext();
                    context.setCurrentThreadContext();
                    }
            ...
            try
                {
                ParameterResolver paramResolver = getParameterResolver(sName, clzMapping, loader, null, context);
                ...
                mapping.preConstruct(context, resolver, dependencies);

                collection = bldrCollection.realize(constraint, resolver, dependencies);

                mapping.postConstruct(context, collection, resolver, dependencies);
                }
            finally
                {
                if (context != null)
            ...
            for (Service service : mapServices.keySet())
                ...
                synchronized (service)
                    ...
                    if (setRefs == null ||
                        ...
                        {
                        // this CCF was the last referrer
                        ContainerContext context = ContainerHelper.getSwitchServiceContext(service);
                        if (context != null)
                            {
                            context.setCurrentThreadContext();
                            }
     ...
     *
     * @return the application Context or null, if none is associated
     */
    public Context getApplicationContext()
        {
        return f_registry.getResource(Context.class);
        }

    /**
     * Return the ContainerContext associated with this ECCF.
     *
     * @return the ContainerContext or null, if none is associated
     */
    public ContainerContext getContainerContext()
        {
        Context ctxApp = getApplicationContext();
        return ctxApp == null ? null : ctxApp.getContainerContext();
        }
    ...
    public void startServices()
        ...
        if (context != null)
            {
            // collect service schemes for all shared mappings
            // (see CacheConfig.validate)
            setSharedScheme = new HashSet<>();
            Set<String> setSharedCaches = context.getSharedCaches();
        ...
        if (context != null && !setSharedScheme.isEmpty())
            {
            // now start shared services in the GLOBAL context
            context = context.getGlobalContext();
            ...
            try
                ...
                for (ServiceScheme scheme : regScheme)
                    ...
                    if (scheme.isAutoStart() && setSharedScheme.contains(scheme.getSchemeName()))
                        {
                        // use the scheme instead of service name to ensure the correct service
                        // is started when there are multiple services with the same service name
                        ensureService(scheme);
                        }
                    }
                }
    ...
    protected static void checkContextPermission(NamedCache cache)
        {
        ContainerContext ctx = ContainerHelper.getSwitchServiceContext(cache.getCacheService());
        if (ctx == null)
            {
            ...
            {
            NamedCache cacheTmp = cache;
            ctx.runInDomainPartitionContext(() -> checkPermission(cacheTmp));
            }
        }
     ...
     * @param ctxContainer the ContainerContext
     *
     * @return the ParameterResolver
     */
    public ParameterResolver getParameterResolver(final String sCacheName, ClassLoader loader,
            BackingMapManagerContext ctxBMM, final ContainerContext ctxContainer)
        {
        return getParameterResolver(sCacheName, ResourceMapping.class, loader, ctxBMM, ctxContainer);
        }
...
*/
    public <M extends ResourceMapping> ParameterResolver getParameterResolver(final String sResourceName, Class<M> clzResourceMapping, ClassLoader loader,
        ...
        ParameterResolver resolverOuter = new ParameterResolver()
            {
            public Parameter resolve(String sName)
                {
                Object oValue = ctxContainer.getCacheAttribute(sResourceName, sName);
                return oValue == null ? null : new Parameter(sName, oValue);
                }
            };
        return new ChainedParameterResolver(resolverOuter, resolver);
        }
    ...
    */
    ...
    public static class DependenciesHelper
        ...
        public static Dependencies newInstance(XmlElement xmlConfig, ClassLoader contextClassLoader,
            ...
            {
            return newInstance(xmlConfig, contextClassLoader, sPofConfigUri, null, null);
            }
    ...
    public static class Manager
        ...
        protected ParameterResolver getResolver(String sName)
            ...
            return getCacheFactory().
                ...
                context.getCacheService().getResourceRegistry().getResource(ContainerContext.class));
            }
