...
import com.tangosol.internal.net.queue.BaseNamedCacheQueue;
...
public class PagedNamedQueue<E>
    ...
    protected QueuePollResult toResult(Binary binary)
        {
        QueuePollResult result = new QueuePollResult(1, binary);
        result.setContextSerializer(getSerializer());
        return result;
        }
     ...
     */
    protected Iterator<Binary> peekAtBucket(int bucketId)
        {
        return peekAtBucket(bucketId, true);
        }
