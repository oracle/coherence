...
/**
 * A primitive specialization of {@link CollectorAggregator} for integers.
 * <p>
...
public class IntCollectorAggregator<K, V, R>
    ...
    /**
     * Construct IntCollectorAggregator instance.
     *
     ...
     */
    public IntCollectorAggregator(RemotePipeline<IntStream> pipeline,
                                  Supplier<R> supplier,
    ...
    public R aggregate(Set<? extends InvocableMap.Entry<? extends K, ? extends V>> setEntries)
        {
        IntStream s = m_pipeline.evaluate(setEntries.stream());
    ...
    public InvocableMap.StreamingAggregator<K, V, R, R> supply()
        {
        return new IntCollectorAggregator<>(m_pipeline, m_supplier, m_accumulator, m_combiner);
        }
