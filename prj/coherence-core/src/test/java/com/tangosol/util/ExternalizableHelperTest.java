...
public class ExternalizableHelperTest extends ExternalizableHelper
    ...
    */
    ...
    public void testConfigGetSerialFilterFactory()
        {
        assumeJDKVersionEqualOrGreater(17);
    ...
    */
    ...
    private static void assumeJDKVersionEqualOrGreater(int nMinVersion)
        {
        String[] asVersion = System.getProperty("java.version").split("-|\\.");
        int      nVersion  = Integer.parseInt("1".equals(asVersion[0]) ? asVersion[1] : asVersion[0]);

        Assume.assumeTrue("Skipping test for Java version " + nVersion
                + " less than required version of " + nMinVersion, nVersion >= nMinVersion);
        }
