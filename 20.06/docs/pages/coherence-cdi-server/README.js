...
<p>It allows you both to inject Coherence-managed resources, such as <code>NamedMap</code>, <code>NamedCache</code> and <code>Scope</code>  instances into
...
<li>
<p><router-link to="#inject-ccf" @click.native="this.scrollFix('#inject-ccf')"><code>ConfigurableCacheFactory</code> and <code>Scope</code> Injection</router-link></p>
...
<p>In this case you would use <code>@Scope</code> qualifier to specify the scope name, or the URI of the  configuration file to use:</p>
...
<h5 id="inject-ccf"><code>ConfigurableCacheFactory</code> and <code>Session</code> Injection</h5>
<div class="section">
<p>On rare occasions when you need to use either of these directly, Coherence CDI makes it trivial to do so.</p>

<p>To obtain an instance of a default <code>ConfigurableCacheFactory</code> or <code>Session</code>, all you need to do is inject them into the
class which needs to use them:</p>
...
<p>If you need a specific <code>ConfigurableCacheFactory</code> or <code>Session</code> you can simply qualify them using <code>@Scope</code> qualifier and
specifying the scope name, or the URI, of the configuration file to use:</p>
...
@Inject
@Scope("my-cache-config.xml")
private ConfigurableCacheFactory session;</markup>
...
<div class="section">
<p>Once the custom annotation has been created a <code>FilterFactory</code> implementation can be created that will be responsible
...
<li data-value="3">The <code>CustomFilterSupplier</code> implements the <code>FilterFactory</code> interface&#8217;s <code>create</code> method where it creates the
...
<div class="section">
<p>Now there is both a custom annotation, and an annotated <code>FilterFactory</code>, the injection point requiring the <code>Filter</code>
