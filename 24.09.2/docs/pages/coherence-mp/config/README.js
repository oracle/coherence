...
>-Dcoherence.cluster=MyCluster -Dcoherence.role=Proxy -Dcoherence.distributed.localstorage=false</markup>
...
<ul class="ulist">
<li>
<p>When you are using one of Eclipse MicroProfile implementations, such as  <a id="" title="" target="_blank" href="https://helidon.io/">Helidon</a> as the foundation of your application, it would be nice to define some of Coherence configuration parameters along with your other configuration parameters, and not in the separate file or via system properties.</p>

</li>
<li>
<p>In some environments, such as Kubernetes, Java system properties are cumbersome to use, and environment variables are a preferred way of passing configuration  properties to containers.</p>
...
<ul class="ulist">
<li>
<p>Unlike pretty much all of the default configuration sources, which are static, configuration options stored in a Coherence cache can be modified without forcing you to rebuild your application JARs or Docker images.</p>

</li>
<li>
<p>You can change the value in one place, and it will automatically be visible and up to date on all the members.</p>
...
<p class="admonition-inline">It should be obvious, but it&#8217;s worth pointing out that you cannot use Coherence cache as a config source for properties such as the one above, or for any other configuration property required during Coherence startup and initialization.</p>
</div>
<p>This feature is primarily intended for easier management of application configuration options that are a) not needed during application startup, and b) would benefit from being mutable at runtime.</p>
</p>
</div>
</div>
</doc-view>
