...
>implementation 'com.oracle.coherence.ce:coherence-grpc-proxy:21.06.1-SNAPSHOT'</markup>


<h3 id="_start_the_server">Start the Server</h3>
<div class="section">
<p>The gRPC server will start automatically when <code>com.tangosol.coherence.net.Coherence</code> (or <code>com.tangosol.coherence.net.DefaultCacheServer</code>) is started.
Typically, <code>Coherence</code> will be used as the application&#8217;s main class, alternatively an instance of <code>Coherence</code> can be started using the bootstrap API.</p>
...
coherence.grpc.server.tls.cert=/certs/server.pem</markup>

<p>An optional server key file password can be provided if required, for example if the key file password is <code>secret</code>:</p>

<markup


>coherence.grpc.server.tls.password=secret</markup>

<p>By default, the server does not require client certificates. The server can be configured to use mutual authentication to verify the client certificates.</p>

<markup


>coherence.grpc.server.tls.client=REQUIRED</markup>

<p>The valid values for the <code>coherence.grpc.server.tls.client</code> system property are <code>NONE</code>, <code>REQUIRED</code>, or <code>OPTIONAL</code>.</p>

<p>If a CA file is required to verify the client certs it can be provided to the server, for example if the CA file is called <code>/certs/ca.pem</code>:</p>

<markup


>coherence.grpc.server.tls.ca=/certs/ca.pem</markup>
